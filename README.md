...
- 比赛元数据保存美国队每局近端/远端的候选有效比赛区间和对应半场裁剪。
...
│  └─ *_match.json               # 比赛信息、当前清单、复核状态与半场候选区间
├─ docs/
...
- 67 个候选窗口的首轮人工复核清单继续保留；17 条二次复核和第一批完整回合补标均已应用，
  当前训练清单为 73 条，比赛状态为 `expansion_batch_01_applied`。
...
比赛元数据的 `usa_side_segments` 现保存以下主动比赛区间候选：
...
这些值来自逐帧视觉检查，精度为 1 秒，状态仍是 `candidate`，不能当作正式裁判记录。
...
它们只描述含有效回合的候选范围；局间、暂停和第 5 局换边空档会保留为空白，不应自动套用
前一段裁剪。现有 20 个复核区间均被同侧候选唯一覆盖。后续整场扫描可以先按这些范围过滤，
但在用户确认边界前必须保留 `requires_user_confirmation: true`。
