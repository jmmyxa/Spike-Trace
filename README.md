...
- 独立验证来源绑定契约：冻结视频 SHA-256/元数据，并对清单与选择源执行显式内容隔离。
- 独立验证回合队列：基于确定性运动候选补齐整场覆盖、处理换边裁剪，并生成静音代理。
- 独立验证真值：以 prediction-blind 草稿逐回合记录动作与可见性，并原子锁定不可变 JSON/CSV 权威 bundle。
...
- 分段推理对侧区间映射、裁剪、参数和解码异常统一 fail-closed 抛出验证错误。
