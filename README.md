...
这些数字仍来自同一场比赛，只是零样本兼容性检查，不是正式模型成绩。它说明当前权重可以辅助发现部分 `serve`、`receive` 和 `block` 候选，但不能直接用于自动统计。下一步必须根据人工备注重新对齐动作中心时刻，补足正样本并建立独立验证比赛，再考虑微调。

## 当前限制

当前只有一场真实比赛和 67 个首轮 reviewed 窗口，没有可用于生产的模型权重。代码已经
