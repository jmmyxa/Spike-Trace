...
class TruthTests(unittest.TestCase):
    def fixture(self):
        ...
        video = root / "match.mp4"; video.write_bytes(b"video")
        metadata = VideoMetadata(video, 1.0, 20, 100, 80, 20.0)
        binding = ValidationVideoBinding("match-1", video, root, "match.mp4", sha256_file(video), metadata)
        segments = (
    ...
    def test_lock_projection_and_verify(self):
        ...
        self.assertEqual(out["csv"],csv_path); self.assertEqual(csv_path.read_bytes()[:3],b"\xef\xbb\xbf"); self.assertEqual(csv_path.read_text(encoding="utf-8-sig").splitlines()[0],CSV_HEADER)
